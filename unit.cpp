...
std::string Unit::icon() const { return text_icon; }

bool Unit::colour() const { return is_black; }

int Unit::type() const { return unit_type; }

std::string Unit::name() const { return "Unit"; }
...
bool Unit::cellHasUnit(const int _row, const int _col) const {
  return (boardptr->board[_row][_col] != NULL);
}

bool Unit::cellHasAlly(const int _row, const int _col) const {
  return (boardptr->board[_row][_col]->colour() == is_black);
}

bool Unit::cellWithinBoard(const int _row, const int _col) const {
...
Unit* Unit::occupyCell(const int _row, const int _col) {
  ...
  if (boardptr->board[_row][_col] != NULL) {
    temp_ptr = boardptr->board[_row][_col];
  }
...
void Unit::addToThreatRange(const int _row, const int _col, std::set<std::pair<int, int> >& _dangerzones) {
...
void Unit::moveRange(std::set<std::pair<int, int> >& _dangerzones) {}
